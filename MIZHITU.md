...
* netty权威指南
* java面试宝典有讲
* 用户线程需不需要等待数据从channel到buffer？（是要监听channel通道吗）
  * 应该是DMA拷贝 ，而不是cpu拷贝吧，应该不需要cpu把
* javaNio中channel通道相当于io数据流和buffer缓存区交换
  * 读数据(数据流动) => channel => buffer（木桶）
    * 非阻塞读取？
    * 只要有一点数据就去读？
      * 监听socket？
  * 写数据(数据流动) => buffer => channel（木桶）
